...
// use CURL
#include <curl/curl.h>
constexpr char *internal_url_start =
    "https://profile-counter.glitch.me/toolsmechta.kz";
...
std::string get_fetch_filename(int type) {
  return std::string(fetch_filename) + std::string(types[type]) +
         fetch_extension;
}
...
int main(int argc, char *argv[]) {
  ...
  for (int x = 0; x < max_tp; ++x) {
    std::cout << "\t" << (types[x] + 1) << " = " << nums[x] << " (count)"
              << std::endl;
  }
  return 0;
}
...
#pragma omp parallel for
  for (size_t tp = 0; tp < max_tp; ++tp) {
    ...
    if (f.is_open()) {
      ...
      f.seekg(len - std::min(static_cast<uint32_t>(sizeof buf), len),
              ios_base::beg);
    ...
    if ((tp_nums[tp] = tp_nums_save[tp] = num) != -1)
      tp_nums[tp] =
          std::abs(prevNum - num) - /*Pop 1 elem for current session*/ 1;
  }
  // save
  save(tp_nums_save);
  return true;
...
int fetch_num(const std::string &url) {
  ...
  int num = parseFromSvg(
      "<?xml version=\"1.0\" encoding=\"UTF-8\"?><svg width=\"224px\" "
